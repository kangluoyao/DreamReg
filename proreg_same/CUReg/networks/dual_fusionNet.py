...
class NLBlockND_cross(nn.Module):
    ...
    def __init__(self, in_channels, inter_channels=None, mode='embedded',
        ...
        self.to_hidden = nn.Sequential(
            nn.Conv3d(self.in_channels, self.in_channels,1,1,0),
            nn.SiLU()
        ) 
...
class RegistNetwork(nn.Module):
    ...
    def forward(self, vol, frame, device=None):
        ...
        fused_fea = self.reduce_layer(densefused_feat).view(Bs,-1,t, h, w).contiguous()
        ...
        return pred_dof, out_frame_tensor_full#, pred_interframe_of, seg_feat
