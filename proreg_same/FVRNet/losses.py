...
class regularization_loss(_Loss):
    def __init__(self):
        ...
        self.ms_ssim_loss = SSIM(data_range=1.0, size_average=True, channel=1)
        self.ms_ssim_loss_r = SSIM(data_range=1.0, size_average=True, channel=1)
...
class CornerDistLoss(nn.Module):
    def __int__(self):
        super().__int__()
    def forward(self, pred_pose, target_pose, device=torch.device("cuda")):
        # five landmarks
        predict = utils.dof6mat_tensor(pred_pose, device=device)
        label = utils.dof6mat_tensor(target_pose, device=device)
        corner1 = torch.tensor([-56 / 2.0, -56 / 2.0, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
        corner2 = torch.tensor([-56 / 2.0, 56 / 2.0, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
        corner3 = torch.tensor([56 / 2.0, -56 / 2.0, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
        corner4 = torch.tensor([56 / 2.0, 56 / 2.0, 0, 1], dtype=torch.float32).unsqueeze(1).to(device)
