...
class mynet3(nn.Module):
    ...
    def __init__(self, layers):
        ...
        self.avgpool = nn.AvgPool3d((2, 2, 2), stride=1)
    ...
    def forward(self, vol, frame, device=None):
        ...
        if show_size:
            ...
            mat = tools.dof2mat_tensor(input_dof=x, device=device,normalize_dof=False)
        ...
        return vol_resampled, x, out_frame_tensor_full


class mynet3_noSim(nn.Module):
    """ First working model! """

    def __init__(self, layers):
        self.inplanes = 64
        super(mynet3_noSim, self).__init__()
        """ Balance """
        layers = layers
        # layers = [3, 4, 6, 3]  # resnext50
        # layers = [3, 4, 23, 3]  # resnext101
    ...
    def forward(self, vol, frame, device=None):
        ...
        if show_size:
            ...
            mat = tools.dof2mat_tensor_nograd(input_dof=x, device=device, normalize_dof=False)
            # indices = torch.tensor([0, 1, 2]).to(device)
            # mat = torch.index_select(mat, 1, indices)
