...
# 空间设置：RAI + spacing=1 + size=(X,Y,Z)
DICOM_ORIENT = "RAI"
SPACING_XYZ  = (1.0, 1.0, 1.0)
SIZE_XYZ     = (128, 128, 32)
SLICE_HW     = (128, 128)               # 你训练/测试都用 128x128
NUM_SLICES_PER_VIEW_VAL  = 4            # 每视角生成的固定切片数（val）
NUM_SLICES_PER_VIEW_TEST = 4            # 每视角生成的固定切片数（test）
SCALER = np.array([10,10,10, 10,10,10], dtype=np.float32)  # [tx,ty,tz,rx,ry,rz], 体素/度
DELTA, LOWER_BOUND = 0.35, 0.0          # 非空判定
...
def generate_fixed_slices_for_patient(pid: str, out_dir: str, num_per_view: int, seed_offset: int):
    """为一个 patient 的 2CH & 4CH 生成固定切片，seed_offset 用于区分 val/test。"""
    ...
    for view, img_fn, msk_fn in views:
        ...
        # 固定随机：不同 split 用不同 offset，保证 val/test 不同但稳定
