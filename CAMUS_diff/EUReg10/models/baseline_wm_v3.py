...
class EUReg_WM_Belief(nn.Module):
    ...
    def __init__(self, vol_shape, in_channel=1,
        ...
        self.inp = nn.Linear(z_dim + 6 + z_dim + z_dim, h_dim)
    ...
    def wm_observe_step(self, vol, goal_z, zv, T_t, a_t, h_prev):
        """
        单步 Dreamer-style 观察（train-time）。
        Inputs:
            vol:    (B,1,D,H,W)   3D US 体数据
            goal_z: (B,z_dim)     目标 slice 的 embedding
            zv:     (B,z_dim)     volume 的 embedding
            T_t:    (B,6)         当前真实 pose
        ...
        with torch.no_grad():
            # transformer 输出 (B,1,1,H,W) 或 (B,1,H,W,?)，这里按你原来的 squeeze(2) 方式
            cur = self.transformer(vol, T_t).squeeze(2)  # (B,1,H,W)

        # 2) encode 当前观测 slice
        z_obs = self._encode_slice(cur)                 # (B,z_dim)

        # 3) prior: p(z_t | h_prev, a_t, goal_z, zv)
        ...
        # 4) posterior: q(z_t | h_prev, a_t, goal_z, zv, z_obs)
        ...
        # 6) 更新 deterministic belief h_t
        gru_inp = torch.cat([z_t, a_t, goal_z, zv], dim=-1)
        # h_t = self.gru(gru_inp, h_prev)                 # (B,h_dim)
    ...
    def wm_imagine_step(self, goal_z, zv, a_t, h_prev):
        """
        想象步（imagination）：只用 prior，不再用 transformer。
        Inputs:
            goal_z, zv: encode_volume/encode_slice 得到的上下文
            a_t:        (B,6) 动作
            h_prev:     (B,h_dim)
        Returns:
            h_t, z_t, sl_pred, r_pred
        """
        # prior p(z_t | h_prev, a_t, goal_z, zv)
        prior_inp = torch.cat([h_prev, a_t, goal_z, zv], dim=-1)
        prior_stats = self.prior_net(prior_inp)
        mu_p, logvar_p = self._split_mu_logvar(prior_stats)
