...
def main():
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            starter.record()
            _, pred_dof, sampled_frame, _, _ = model(vol, frame, device=device)
            ender.record()
            ...
            plt.imsave(pic_path+'{:03d}_dist_{:.2f}_ncc_{:.2f}.png'.format(stdy_idx, dist_err, ncc), sampled_frame.squeeze().detach().cpu().numpy(), cmap='gray')
            stdy_idx += 1
    print(save_dir)
