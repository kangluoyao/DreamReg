...
import losses
import utils
from models.baseline_wm import EUReg_WM_Belief   # world model
...
def main():
    ...
    img_size = (32, 192, 192)
    save_dir = 'save_dir/'
    model_dir = os.path.join('experiments/CAMUS2/', save_dir)
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            vol   = data[0].to(device, non_blocking=True)
            frame = data[1].to(device, non_blocking=True)
            dof   = data[2].to(device, non_blocking=True)
