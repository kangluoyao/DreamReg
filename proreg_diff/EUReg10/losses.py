...
class CornerDistLoss(nn.Module):
    ...
    def forward(self, pred_pose, target_pose):
        ...
        predict = self.dof2mat(pred_pose)
        label = self.dof2mat(target_pose)
        corner1 = torch.tensor([-56 / 2.0, -56 / 2.0, 0, 1], dtype=torch.float32, device=pred_pose.device).unsqueeze(
            1)
        corner2 = torch.tensor([-56 / 2.0, 56 / 2.0, 0, 1], dtype=torch.float32, device=pred_pose.device).unsqueeze(1)
        corner3 = torch.tensor([56 / 2.0, -56 / 2.0, 0, 1], dtype=torch.float32, device=pred_pose.device).unsqueeze(1)
        corner4 = torch.tensor([56 / 2.0, 56 / 2.0, 0, 1], dtype=torch.float32, device=pred_pose.device).unsqueeze(1)
