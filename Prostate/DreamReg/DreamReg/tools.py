...
import tools
from networks import transformations as tfms
import random
...
def dof2mat_tensor(input_dof, device, normalize_dof=False, dof_means=None, dof_std=None):
    ...
    M = torch.zeros((output_dof.shape[0], 4, 4)).cuda()
...
def dof2mat_tensor_nograd(input_dof, device, normalize_dof=False, dof_means=None, dof_std=None):
    if normalize_dof:
        # LL
        dof_means = torch.tensor(dof_means).to(device)
        dof_std = torch.tensor(dof_std).to(device)
    ...
    M = torch.zeros((output_dof.shape[0], 4, 4), device='cuda', dtype=input_dof.dtype)
    M.requires_grad = True
    with torch.no_grad():
        M[:, 0, 0] = cj * ck
        M[:, 0, 1] = sj * sc - cs
