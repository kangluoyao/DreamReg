...
def main():
    val_dir = '/Media_HDD/lykang/dataset/CAMUS_public/splits/testing/'
    dim=12
    save_dir = 'EUReg10_1/'
    img_size = (32, 128, 128)
    slice_size = (128, 128)
    ...
    Initialize model
    '''
    model = EUReg_FRT_Test(img_size, dim=dim)
    model.cuda()
    ...
    print(model_dir + 'best_model.pth.tar')
    del best_model
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            stdy_idx += 1
    log_str = ('DistErr: {:.2f} +- {:.2f} mm, NCC: {:.2f} +- {:.2f} %, SSIM: {:.2f} +- {:.2f} %, '
